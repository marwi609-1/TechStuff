"""Erzeugt den Wochenreport als HTML aus lokalen Intervals-Exporten.
    ...
    python3 scripts/weekly_report.py [--week-end YYYY-MM-DD] [--out data/report.html]

Eingaben (in data/, nicht versioniert):
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=DATA / "report.html")
    ...
    args.out.write_text(render_html(summary, activities), encoding="utf-8")
    print(f"KW {summary.week_start.isocalendar().week}: {args.out}")
    print(
